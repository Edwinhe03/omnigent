...
# Pin ucode to the exact commit the managed lakebox image ships (its "post5"
# wheel, built from ucode main @ 94271a7) so the OSS/uvx path stays reproducible
# and in lockstep with managed instead of drifting with ucode's ``main`` HEAD.
# A full SHA is immutable, so uvx caches the built wheel by ref and reuses it
# across runs — no ``--refresh-package`` needed to defeat a mutable branch cache.
...
def find_ucode_command() -> list[str]:
    ...
    Prefers an ephemeral ``uvx`` run pinned to the fixed ucode commit the
    managed lakebox image ships, so setup uses a known-good ucode rather than
    whatever (possibly stale) ``ucode`` the user installed long ago. A
    locally-installed binary is used only as a last resort when ``uvx`` is
    unavailable. This ordering matters: an old persistently-installed ``ucode``
    predates options like ``configure --workspaces`` and would otherwise win
    and break setup.
