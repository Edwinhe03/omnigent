...
async def _ensure_local_codex_resume_rollout(
    ...
    *,
    ...
    known-thread terminal we make one bounded attempt to commit any
    proven-undelivered completed items, then rewrite the rollout from committed
    Omnigent items. This keeps the server transcript authoritative without
    allowing an old dead letter to append after newer live turns. If server
    history is temporarily unavailable, a valid local rollout remains a
    best-effort fallback. A successful server fetch wins even when its committed
    history is empty or shorter than the local file; local-only records are
    ...
    await recover_codex_transcript_dead_letters_before_resume(
        client,
        bridge_dir=codex_home.parent,
        session_id=session_id,
    )
    try:
